...
let parser_results
function read_data() {
    const table = window.sessionStorage.getItem('table')
    if (!table) {
        alert('Sorry, you don\'t have any data loaded for this session! Go to the *data* page and insert some.')
        return
    }
    parser_results = lesson_to_json(table.substring(table.indexOf('<tbody>') +  7).trim().replaceAll('<tr>', '').replaceAll('<td>', '').replaceAll('</td></tr>', '\r\n').replaceAll('</td>', ',').replace('</tbody>', ''))
}
